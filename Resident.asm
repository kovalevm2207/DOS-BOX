...
; Входные параметры: bp - указатель на число
...
;---------------------------------------------------------------------------------------
; New08: прерывание, вызывающее из под себя функцию для отображения рамки со значениями
;        регистров вне этого прерывания
; Входные параметры: --//--,
; Ожидаемое состояние: cs - сегмент с кодом программы Resident.asm
;                      Old08 - содержит значение из таблицы прерываний на для исполняе-
...
New08   proc    ; pushf     ; sp = real_sp - 2
        ...
        push    di si dx cx bx ax       ; <=> push sp+6 ss es ds bp di si dx cx bx ax
